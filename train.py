...
if __name__ == '__main__':
    ...
    parser.add_argument('--start_epoch', type=int, default=1, 
                        help="Starting epoch (set automatically when resuming)")
    ...
    # Determine CSV file names (capitalization differs between the two scripts)
    train_csv = './train.csv'
    val_csv = './val.csv'
    # Check if files exist, fallback to uppercase if needed
    if not os.path.exists(train_csv):
        train_csv = './Train.csv'
    if not os.path.exists(val_csv):
        val_csv = './Val.csv'

    train_dataset = cpuDataset(train_csv, attributes, train_transform)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)

    val_dataset = cpuDataset(val_csv, attributes, val_transform)
    ...
    if args.resume and args.resume_from_checkpoint:
        ...
        try:
            ...
            if checkpoint_filename.startswith('checkpoint-') and checkpoint_filename.endswith('.pth'):
                epoch_str = checkpoint_filename.replace('checkpoint-', '').replace('.pth', '')
                start_epoch = int(epoch_str) + 1
                print(f"Resuming from epoch {start_epoch}")
        except:
            print(f"Could not determine epoch from checkpoint filename. Starting from epoch {start_epoch}")
